...
#We found that the variation in this column is from 102(Lives) to 63(Dies)
...
#We consider that outliers represent natural variability in the process being trained, removing them may result in the loss of important information
...
models.update({"Decision Tree (Gini)":dt_model})
...
models.update({"Random Forest":rf_model})
...
# Create an encoder to transform categorical columns into dummy variables (because our categorical columns were still being recognized as an object and it gives conflict in this model)
...
models.update({'XGBoost':xgb_model})
...
# Calculate accuracy on the training set
xgb_acc_train = sum(xgb_preds_train == y_train) / len(y_train)
...
models.update({"Logistic Regression":lr_model})
...
# Apply MinMaxScaler (We can't use the MinMaxScaler() made above)
scaler = MinMaxScaler()
X_train_scaled = scaler.fit_transform(X_train)
X_test_scaled = scaler.transform(X_test)
...
models.update({'Neural Network':mlp_model})

# Predict on training set
mlp_preds_train = mlp_model.predict(X_train_scaled)

# Calculate accuracy on the training set
mlp_acc_train = np.mean(mlp_preds_train == y_train)

# Predict on the test set
mlp_preds_test = mlp_model.predict(X_test_scaled)

# Calculate accuracy on the test set
mlp_acc_test = np.mean(mlp_preds_test == y_test)

print("Train Accuracy Neural Network: {:.3f}".format(mlp_acc_train))
print("Test Accuracy Neural Network: {:.3f}".format(mlp_acc_test))
...
# Calculate accuracy on the training set
knn_acc_train = np.mean(knn_preds_train == y_train)

# Predict on the test set
knn_preds_test = knn_model.predict(X_test_scaled)

# Calculate accuracy on the test set
knn_acc_test = np.mean(knn_preds_test == y_test)

print("Train Accuracy KNN: {:.3f}".format(mlp_acc_train))
print("Test Accuracy KNN: {:.3f}".format(knn_acc_test))

#Data Evaluation

from sklearn.metrics import confusion_matrix, roc_curve, auc, precision_recall_curve

#Training and testing data from different models
model_names = ["Decision Tree (Gini)", "Random Forest", "SVM", "XGBoost", "Logistic Regression", "Neural Network",
               "KNN"]
train_accuracies = [0.730, 1.000, 0.917, 0.985, 0.826, 1.000, 1.000]
test_accuracies = [0.680, 0.760, 0.697, 0.667, 0.636, 0.680, 0.640]
...
# Define the models and their test predictions
model_predictions = {
    "Decision Tree (Gini)": dt_preds_test,
    "Random Forest": rf_preds_test,
    "SVM": svm_preds_test,
    "XGBoost": xgb_preds_test,
    "Logistic Regression": lr_preds_test,
    "Neural Network": mlp_preds_test,
    "KNN": knn_preds_test
}
#we printed this in an attempt to detect the error
# Debug predictions for SVM
print("SVM - Test Data Length:", len(X_test_scaled))
print("SVM - Predictions Length:", len(svm_preds_test))

# Debug predictions for XGBoost
print("XGBoost - Test Data Length:", X_test_encoded.shape[0])
print("XGBoost - Predictions Length:", len(xgb_preds_test))

# Debug predictions for Logistic Regression
print("Logistic Regression - Test Data Length:", X_test_scaled.shape[0])
print("Logistic Regression - Predictions Length:", len(lr_preds_test))

# Train models and make predictions
'''proba_predictions = {}
for model_name, model in models.items():
    if model_name == "Neural Network" or model_name == "KNN":
        model.fit(X_train_scaled, y_train)
        proba_predictions[model_name] = model.predict_proba(X_test_scaled)
    elif model_name == "Logistic Regression" or model_name == "SVM":
        model.fit(X_train_scaled, y_train_encoded)
        proba_predictions[model_name] = model.predict_proba(X_test_encoded)
    elif model_name == "XGBoost":
        model.fit(X_train_encoded, y_train_encoded)
        proba_predictions[model_name] = model.predict_proba(X_test)
    else:
